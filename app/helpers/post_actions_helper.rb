module PostActionsHelper
  ...
  def favorite_link(user, post)
    if post.favorites_count?
      ...
      if favorite.nil?
        link_to((image_tag 'unfavorite.png', :height => "7.5%", :width => "7.5%"), favorites_path(:post_id => post), :method => :post, :remote => true)
      else
        link_to((image_tag 'favorite.png', :height => "7.5%", :width => "7.5%"), favorite, :method => :delete, :remote => true)
      end
    else
      link_to((image_tag 'unfavorite.png', :height => "7.5%", :width => "7.5%"), favorites_path(:post_id => post), :method => :post, :remote => true)
    end
